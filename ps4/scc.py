...
def compute_strong_components(graph, reversed_graph):
	'''Kosaraju's algorithm for computing strongly connected components in a directed graph'''
	global finish_order
...
def depth_first_search(graph, start_vertex, track_finish_order, track_leader):
	...
	# Have to do this iteratively or Python explodes, even after increasing recursion limit
	stack = [start_vertex]
...
def build_graph(list):
	...
	for item in list:
		...
		if not head in graph: add_new_vertex(graph, head)
		if not tail in graph: add_new_vertex(graph, tail, head)
		else: graph[tail]['data'].append(head)
	return graph

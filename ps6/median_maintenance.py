...
def median_maintenance(num_stream):
	...
	for num in num_stream[2:]:
		if num <= -q_left.queue[0]: q_left.put(-num)
		else: q_right.put(num)

		if abs(q_right.qsize() - q_left.qsize()) > 1:
			if q_left.qsize() > q_right.qsize(): q_right.put(-q_left.get())
			else: q_left.put(-q_right.get())
		

	return -q_left.queue[0], q_right.queue[0]

def tests():
	lst = [5,2,1,8,3,9,4,7,6,10]
